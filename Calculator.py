...
root = Tk()
root.title("Simple Calculator")#sets the title of the window as simple calucaltor
...
def click(event):#defines a function to handel button clicks
    text = event.widget.cget("text")#retrive the text on the clickes button
    if text == "=":
        try:
...
button_frame = Frame(root, bg="#17161b")#creat a frame widget to contain the buttons ad set bacjground color.
...
row = 0  #initialize row and coloumns counters for button placement.
col = 0
...
for button in buttons:  #iterates through the list of button labels.
    ...
    col += 1  #moves to the next coloumn
    if col > 3:
        col = 0
        row += 1

# Main loop to run the application
root.mainloop()  #starts the main event loop ,wait for user interactionss
